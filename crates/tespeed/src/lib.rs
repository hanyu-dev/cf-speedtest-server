...
const MAX_ALLOWED_DECOMPRESSION_BOMB_SIZE: u64 = 10 * 1024 * 1024 * 1024; // 10 GiB

#[derive(Debug, Clone, Copy)]
/// Decompression bomb.
pub struct DecompressionBomb {
    size: NonZeroU64,
}

impl Default for DecompressionBomb {
    fn default() -> Self {
        Self {
            #[allow(unsafe_code, reason = "XXX")]
            size: unsafe { NonZeroU64::new_unchecked(200 * 1024 * 1024) },
        }
    }
}

impl DecompressionBomb {
    /// Create a new [`DecompressionBomb`] with the specified size.
    ///
    /// When `size` is `None`, the default size of 200 MiB will be used.
    pub const fn new(size: NonZeroU64) -> Result<Self, TooLargeDecompressionBombSize> {
        if size.get() > MAX_ALLOWED_DECOMPRESSION_BOMB_SIZE {
            Err(TooLargeDecompressionBombSize)
        } else {
            Ok(Self { size })
        }
    }

    /// Get the target size of the decompression bomb in bytes.
    ...
    pub fn zstd(&self) -> Vec<u8> {
        ...
        let fcs_flag = {
            ...
            let fcs_flag = match self.size.get() {
                0 => FCS_FLAG_0,
        ...
        let window_size = {
            ...
            let window_exponent = match self.size.get() {
        ...
        match fcs_flag {
            FCS_FLAG_0 => {
                output.push(self.size.get() as u8);
            }
            FCS_FLAG_1 => {
                output.extend(&((self.size.get() - 256) as u16).to_le_bytes());
            }
            FCS_FLAG_2 => {
                output.extend(&(self.size.get() as u32).to_le_bytes());
            }
            FCS_FLAG_3 => {
                output.extend(&self.size.get().to_le_bytes());
            }
            _ => unreachable!(),
        }
        ...
        while remaining > 0 {
            ...
            // RLE byte (0x00)
            let block =
                (last_block | (BLOCK_TYPE_RLE << 1) | ((block_size as u32) << 3)).to_le_bytes();
...
#[derive(Debug)]
/// The requested decompression bomb size is too large.
pub struct TooLargeDecompressionBombSize;

impl Display for TooLargeDecompressionBombSize {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "the requested decompression bomb size is too large.")
    }
}

impl Error for TooLargeDecompressionBombSize {}
